...
class HttpRequester {
  ...
  async fetch<T>(url: string): Promise<T> {
    if (this.adapter instanceof RestApiAdapter) {
      const response = await makeRestApiCall<T>(url);
      // transform response and return
    } else if (this.adapter instanceof GraphQLAdapter) {
      const response = await makeGraphQLCall<T>(url);
      // transform response and return
    }
  }
}

function makeRestApiCall<T>(url: string): Promise<T> {
  // request and return promise
}

function makeGraphQLCall<T>(url: string): Promise<T> {
  // request and return promise
}
```
...
const shapes = [new Rectangle(10, 20), new Square(5)];
renderShapes(shapes);
```
